...
function summarizeStep(node: string, data: Record<string, any>): string {
  switch (node) {
    ...
    case 'verify_calculation': {
      ...
      const apStr =
        ap.applicable === false || ap.applicable === undefined ? 'n/a' : ap.satisfied ? 'ok' : 'fail'
    ...
    <div className="animate-fade-up rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-2.5 space-y-1.5 text-sm text-slate-700">
        <p className="flex items-center gap-1.5">
          ...
          {sentTo ? (
            <>
              Email sent to <span className="font-mono text-xs">{sentTo}</span>
            </>
          ) : (
            'Reviewer notified'
          )}
        ...
        {decision.reviewer_action === 'approved' && (
          <p className="animate-fade-up font-medium text-emerald-600">
            ✅ Approved {decision.reviewer_action_at ? timeAgo(decision.reviewer_action_at) : ''}
          </p>
        )}
        {decision.reviewer_action === 'rejected' && (
          <p className="animate-fade-up font-medium text-rose-600">
            ❌ Rejected {decision.reviewer_action_at ? timeAgo(decision.reviewer_action_at) : ''}
          </p>
        )}
        {decision.reviewer_action === 'request_info' && (
          <p className="animate-fade-up font-medium text-amber-600">
            ✋ More info requested{' '}
            {decision.reviewer_action_at ? timeAgo(decision.reviewer_action_at) : ''}
          </p>
        )}
      </div>
    </div>
...
export function AgentPanel({ adm }: Props) {
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'step') {
        ...
        if (key) {
          setSteps((prev) => ({
            ...prev,
            [key]: { state: 'complete', summary: summarizeStep(msg.node, data) },
          }))
        }
        return
      }
  ...
  return (
    <div className="w-100 shrink-0 overflow-y-auto border-l border-slate-200 bg-slate-50/80 p-6">
      <h2 className="text-xs font-semibold uppercase tracking-widest text-slate-400">Agent</h2>

      <button
        type="button"
        disabled={!adm || status === 'running'}
        onClick={handleRun}
        className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-slate-800 active:scale-[0.99] disabled:cursor-not-allowed disabled:bg-slate-300"
      >
      ...
      <div className="mt-5 space-y-5">
        {status === 'idle' && (
          <p className="text-sm text-slate-400">Run the agent to evaluate this ADM.</p>
        )}

        {status === 'not_implemented' && (
          <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-600">
            Agent not implemented yet — the LangGraph state machine is Day 2 work. This button is
            wired to the real endpoint and will work once that ships.
          </div>
        )}
        ...
        {status === 'done' && decision && (
          <div className="space-y-5">
            {(() => {
              ...
              return (
                <div
                  className={`animate-fade-up rounded-xl border bg-linear-to-b p-6 text-center shadow-sm ${meta.card}`}
                >
                  <div
                    className={`animate-pop-in mx-auto flex h-12 w-12 items-center justify-center rounded-full shadow-sm ${meta.icon_bg}`}
                  >
                  ...
                  <div className="mx-auto mt-5 max-w-55">
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-200/70">
                      <div
                        className={`animate-grow-bar h-full rounded-full ${meta.bar}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
            ...
            <div className="animate-fade-up" style={{ animationDelay: '80ms' }}>
              ...
              <pre className="mt-2 max-h-64 overflow-y-auto whitespace-pre-wrap rounded-xl border border-slate-200 bg-white p-4 font-mono text-xs leading-relaxed text-slate-800 shadow-sm">
